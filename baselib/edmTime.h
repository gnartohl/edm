/*
 *      stollen from osiTime.h,v 1.6 1997/06/13 09:31:46 jhill Exp
 *
...
#ifndef edmTimehInclude
#define edmTimehInclude
...
class epicsShareClass edmTime {
	friend inline edmTime operator+ 
		(const edmTime &lhs, const edmTime &rhs);
	friend inline edmTime operator- 
		(const edmTime &lhs, const edmTime &rhs);
	...
	friend inline int operator< 
		(const edmTime &lhs, const edmTime &rhs);
public:
	edmTime () : sec(0u), nSec(0u) {}
	edmTime (const edmTime &t) : sec(t.sec), nSec(t.nSec) {}
	edmTime (const unsigned long secIn, const unsigned long nSecIn) 
	{
	...
	static edmTime getCurrent();

	edmTime operator+= (const edmTime &rhs);
	edmTime operator-= (const edmTime &rhs);
...
inline edmTime operator+ (const edmTime &lhs, const edmTime &rhs)
{
	return edmTime(lhs.sec + rhs.sec, lhs.nSec + rhs.nSec);	
}

inline edmTime edmTime::operator+= (const edmTime &rhs)
{
	*this = *this + rhs;
	return *this;
}
...
inline int operator <= (const edmTime &lhs, const edmTime &rhs)
{
	int	rc;
//
