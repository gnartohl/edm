...
//printf( "bgRuleCb, ruleId = %-d, value = %-d\n", ruleId, value );
  ...
  if ( aro->active ) {
    ...
    if ( value != -1 ) {
      ...
      color = aro->actWin->ci->getPixelByIndex( value );
      aro->fillColor.changeColor( color, aro->actWin->ci );
    }
    else {
      //printf( "set normal color\n" );
      aro->fillColor.changeColor( aro->fillColor.pixelColor(),
       aro->actWin->ci );
    }
    aro->needRefresh = 1;
...
expRectClass::expRectClass ( void ) {
  ...
  bgRule->init( (void *) this, BG_RULE, bgRuleCb );
