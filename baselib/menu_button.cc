...
activeMenuButtonClass *mbto = (activeMenuButtonClass *) client;
  ...
  mbto->fgColor.setColor( mbto->bufFgColor, mbto->actWin->ci );
  ...
  mbto->bgColor.setColor( mbto->bufBgColor, mbto->actWin->ci );
...
char oneName[39+1];
  ...
  if ( ( major > 1 ) || ( minor > 0 ) ) {
    ...
    topShadowColor = WhitePixel( actWin->display(),
     DefaultScreen(actWin->display()) );

    botShadowColor = BlackPixel( actWin->display(),
     DefaultScreen(actWin->display()) );
...
//----------------------------------------------------------------------------
  ...
  if ( ni ) {
    ...
    XtSetArg( args[n], XmNtopShadowColor, (XtArgVal) topShadowColor ); n++;
