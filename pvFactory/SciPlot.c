...
EraseAll (SciPlotWidget w)
{
...
#ifdef DOUBLE_BUF
    {
        XSetForeground(XtDisplay(w), 
                       w->plot.defaultGC,
                       w->plot.colors[w->plot.BackgroundColor]);
        XFillRectangle(XtDisplay(w), w->plot.double_buf,
                       w->plot.defaultGC,
                       0, 0,
                       w->core.width,
                       w->core.height);
...
DrawCartesianAxes (SciPlotWidget w)
{
    ...
    if (w->plot.XAxisNumbers) {
        sprintf(label, numberformat, val);
        TextSet(w, x, w->plot.y.AxisPos, label, w->plot.TextColor,
                w->plot.axisFont);
    }
    ...
    if (w->plot.YAxisNumbers) {
        ...
        if (w->plot.YNumHorz) {
            ...
            TextSet(w,
                    ...
                    y, label, w->plot.TextColor,
                    w->plot.axisFont);
        }
        else {
            VTextSet(w, w->plot.x.AxisPos, y, label, w->plot.TextColor,
                     w->plot.axisFont);
        }
    }
    majorval = val;
...
DrawPolarAxes (SciPlotWidget w)
{
    ...
    for (i = 1; i <= w->plot.x.MajorNum; i++) {
        ...
        if (w->plot.XAxisNumbers) {
            ...
            TextSet(w, x2, y2 + height, label, w->plot.TextColor,
                    w->plot.axisFont);
        }
    }
    ...
    if (w->plot.ShowTitle)
        TextSet(w, w->plot.x.TitlePos, w->plot.y.TitlePos,
                w->plot.plotTitle, w->plot.TextColor, w->plot.titleFont);
}
...
#ifdef DOUBLE_BUF
    XCopyArea(XtDisplay(w), w->plot.double_buf,
              XtWindow(w),
              w->plot.screenGC,
              0, 0,
...
void SciPlotSetTextColor (Widget wi, int color)
{
    SciPlotWidget w;
