    ...
    get_element(int opnd, const char **ppsrc, const ELEMENT **ppel)
{
    ...
    *ppel = NULL;
	...
	if (strncasecmp(*ppsrc, pel->name, len) == 0) {
	    *ppel = pel;
	    *ppsrc += len;
	    return TRUE;
	}
 ...
 */
long edm_postfix(const char *psrc, char * const ppostfix, short *perror)
{
    ELEMENT stack[CALCPERFORM_STACK+1];
