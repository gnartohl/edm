...
int ret_stat, stat;
  ...
  data_port = (RPC_PORT) thread_get_app_data( thread_h );
  if ( !data_port ) {
  ...
  stat = nsv_disconnect( data_port );
  if ( ! ( stat & 1 ) ) {
    ret_stat = stat;
  }

  stat = nsv_delete_port( &data_port );
  if ( ! ( stat & 1 ) ) {
    ret_stat = stat;
  }
...
RPC_PORT connect_port, data_port;
THREAD_HANDLE thread_h;
int stat, num_errors;
char optional_data[16+1];

  stat = nsv_create_named_port( "TEST", 1, 128, "my connect port",
   &connect_port );
  if ( ! ( stat & 1 ) ) {
    exit( stat );
  }
  ...
  do {
    ...
    stat = nsv_create_port( 10, 512, "my data port", &data_port );
    if ( ! ( stat & 1 ) ) {
      goto err_label;
    }

    stat = nsv_accept_connection( connect_port, data_port, optional_data );
    if ( ! ( stat & 1 ) ) {
      goto err_label;
    }
    ...
    thread_create_proc( thread_h, rpc_thread );
    if ( ! ( stat & 1 ) ) {
      goto err_label;
    }
...
err_label:
  ...
  stat = nsv_delete_port( &connect_port );
  if ( ! ( stat & 1 ) ) {
    printf( "stat = %d\n", stat );
  }
